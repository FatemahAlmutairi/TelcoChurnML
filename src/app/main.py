...
def get_prediction(data: CustomerData):
    try:
        # Convert Pydantic model to dict and call inference pipeline (Pydantic v2 model_dump or v1 dict)
        result = predict(data.model_dump() if hasattr(data, 'model_dump') else data.dict())
        return {"prediction": result}
    except Exception as e:
        # Return error details for debugging (consider logging in production)
        return {"error": str(e)}

# FIXED: Indentation corrected so this function is at the module level, not inside get_prediction
...
demo = gr.Interface(
    ...
    **Predict customer churn probability using machine learning**
    ...
    """,
    examples=[  # High churn risk example
